<?php echo 'www.lovediscuz.com ';exit;?>
<div id="threadlist" class="tl bm bmw"{if $_G['uid']} style="position: relative;"{/if}>
	...
	<div class="th mi_topinfo mitops cl">
		...
		<!--{else}-->
			<span class="mi_f14">{lang title}</span>
		<!--{/if}-->
	</div>
	<div class="bm_c" style="padding:0;"<!--{if !(empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle'])}--><!--{/if}-->>
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" id="threadlisttableid">
				<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
					...
					<tr>
						<td>
							<div class="mi_postlist cl"<!--{if !(empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle'])}--> <!--{/if}-->>
								<div class="mi_listtx">
									<a href="home.php?mod=space&uid=$announcement[authorid]" c="1" target="_blank"><!--{avatar($announcement[authorid],small)}--></a>
								</div>
								<h2 class="cl">
									<span class="mi_icn"><img src="{IMGDIR}/ann_icon.gif" alt="{lang announcement}" /></span>
									<span class="mi_common"><strong class="xst">{lang announcement}: <!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank">$announcement[subject]</a><!--{else}--><a href="$announcement[message]" target="_blank">$announcement[subject]</a><!--{/if}--></strong></span>
								</h2>
								<p>
									<em class="mi_user"><a href="home.php?mod=space&uid=$announcement[authorid]" c="1">$announcement[author]</a></em>
									<em>$sxp_lang[10] $announcement[starttime]</em>
								</p>
							</div>
						</td>
					</tr>
...
<tbody>
<tr class="ts">
<!--{if $_G['forum']['ismoderator'] && !$_G['gp_archiveid']}--><!--{/if}-->
...
<!--{/if}-->
							...
							<div class="mi_postlist cl">
								<div class="mi_listtx"<!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--> style="position:relative;"<!--{/if}-->>
									<!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}-->
									<span class="mi_o">
									...
									<!--{/if}-->
									<a href="home.php?mod=space&uid=$thread[authorid]" class="mi_listtxs" target="_blank"><!--{avatar($thread[authorid],small)}--></a>
								</div>
								<h2 class="cl">
									<span class="mi_common">
										...
										<!--{if $_G['setting']['threadhidethreshold'] && $thread['hidden'] >= $_G['setting']['threadhidethreshold']}--><span class="mi_hs">{lang hidden}</span>&nbsp;<!--{/if}-->
										<!--{if $thread[icon] >= 0}-->
										...
										<!--{if $thread['rushreply']}-->
											...
											<span id="rushtimer_$thread[tid]" class="mi_tc"> {lang havemore_special} <span id="rushtimer_body_$thread[tid]"></span> <script language="javascript">settimer($rushinfo[$thread[tid]]['timer'], 'rushtimer_body_$thread[tid]');</script>{if $rushinfo[$thread[tid]]['timertype'] == 'start'} {lang header_start} {else} {lang over} {/if} {lang right_special}</span>
											<!--{/if}-->
										<!--{/if}-->
										<!--{if $stemplate && $sortid}-->$stemplate[$sortid][$thread[tid]]<!--{/if}-->
										<!--{if $thread['readperm']}--> - <span class="mi_tc">[{lang readperm} <b>{$thread[readperm]}</b>]</span><!--{/if}-->
										<!--{if $thread['price'] > 0}-->
											<!--{if $thread['special'] == '3'}-->
											- <a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=specialtype&specialtype=reward$forumdisplayadd[specialtype]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}&rewardtype=1" title="{lang show_rewarding_only}"><span class="mi_tc">[{lang thread_reward} <b>$thread[price]</b> {$_G[setting][extcredits][$_G['setting']['creditstransextra'][2]][unit]}{$_G[setting][extcredits][$_G['setting']['creditstransextra'][2]][title]}]</span></a>
											<!--{else}-->
											- <span class="mi_tc">[{lang price} <b>$thread[price]</b> {$_G[setting][extcredits][$_G['setting']['creditstransextra'][1]][unit]}{$_G[setting][extcredits][$_G['setting']['creditstransextra'][1]][title]}]</span>
											<!--{/if}-->
										<!--{elseif $thread['special'] == '3' && $thread['price'] < 0}-->
											- <span class="mi_tc"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=specialtype&specialtype=reward$forumdisplayadd[specialtype]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}&rewardtype=2" title="{lang show_rewarded_only}">[{lang reward_solved}]</a></span>
										<!--{/if}-->
										...
										<!--{if $thread['replycredit'] > 0}-->
											- <span class="mi_tc">[{lang replycredit} <strong> $thread['replycredit']</strong> ]</span>
										<!--{/if}-->
										...
										<!--{/if}-->
										<!--{if CURMODULE == 'guide'}-->
											<span class="mi_tc"><a href="forum.php?mod=forumdisplay&fid=$thread[fid]" target="_blank">$forumnames[$thread[fid]]['name']</a></span>
										<!--{/if}-->
									</span>
								</h2>
								<p>
									<span class="y">
									<em class="mi_view"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></em>
									<em class="mi_reply"><a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra">$thread[allreplies]</a></em>
									</span>
									...
									<em><span{if $thread['istoday']} class="xi1"{/if}>$thread[dateline]</span> $sxp_lang[3]<!--{if $thread[typehtml] || $thread[sorthtml]}-->$sxp_lang[4]<!--{/if}--></em>
									<!--{if $thread[typehtml]}-->
									...
									<!--{/if}-->
									<em>$sxp_lang[5] <a href="{if $thread[digest] != -2 && !$thread[ordertype]}forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost$highlight#lastpost{else}forum.php?mod=viewthread&tid=$thread[tid]{if !$thread[ordertype]}&page={echo max(1, $thread[pages]);}{/if}{/if}" >$thread[lastpost]</a></em>
									<em>
										<a href="forum.php?mod=viewthread&tid=$thread[icontid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra" title="{if $thread['displayorder'] == 1}{lang thread_type1} - {/if}
											...
											{if $thread['replies'] == '0' && $thread['dbdateline']>time()-43200}$sxp_lang[6] - {/if}
	...
	<!--{if $multipage && $filter != 'hot'}-->
		<!--{if !($_G[forum][picstyle] && !$_G[cookie][forumdefstyle])}-->
			<a href="javascript:;" rel="$multipage_more" curpage="$page" id="autopbn" totalpage="$maxpage" picstyle="$_G[forum][picstyle]" forumdefstyle="$_G[cookie][forumdefstyle]"><i></i>$sxp_lang[8]</a>
			<script type="text/javascript" src="{$_G[setting][jspath]}autoloadpage.js?{VERHASH}"></script>
		<!--{else}-->
			<div id="pgbtn" class="pgbtn"><a href="forum.php?mod=forumdisplay&fid={$_G[fid]}&filter={$filter}&orderby={$_GET[orderby]}{$forumdisplayadd[page]}&{$multipage_archive}&page=$nextpage" hidefocus="true">$sxp_lang[9]</a></div>
		<!--{/if}-->
	<!--{/if}-->
	<div class="mi_pgs cl">	
...
<!--{if !IS_ROBOT}-->
	<div id="filter_special_menu" class="p_pop mi_pop" style="display:none" change="location.href='forum.php?mod=forumdisplay&fid=$_G[fid]&filter='+$('filter_special').value">
		<ul>
	...
	<div id="filter_reward_menu" class="p_pop mi_pop" style="display:none" change="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=specialtype&specialtype=reward$forumdisplayadd[specialtype]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}&rewardtype='+$('filter_reward').value">
		<ul>
